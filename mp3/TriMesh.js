...
/** @global Variables for placing the teapot in the desired position */
var teapot_center = [0.0,0.0,0.0]
 ...
 */
    ...
    */
    computeAABB(){
        ...
        // Initialize min and max values with first vertex
        for (var j=0; j<3; j++)
        {
            this.minXYZ[j] = this.vBuffer[j]
            this.maxXYZ[j] = this.vBuffer[j]
        }
        
        
        // Update values iterating through the remaining vertices
        for (var i=1; i<this.numVertices; i++)
        {
            for (var j=0; j<3; j++)
            {
                if(this.vBuffer[3*i+j]<this.minXYZ[j])
                    this.minXYZ[j] = this.vBuffer[3*i+j]
                    
                if(this.vBuffer[3*i+j]>this.maxXYZ[j])
                    this.maxXYZ[j] = this.vBuffer[3*i+j]
            }
        }
    }
    ...
    */
    loadFromOBJ(fileText)
    {
        ...
        for (var i=0; i<lines.length; i++)
        {
            if(lines[i][0]=='v')
            {
                var vertexCoords = lines[i].split(" ");
                this.vBuffer[3*this.numVertices] = parseFloat(vertexCoords[1]);
                this.vBuffer[3*this.numVertices+1] = parseFloat(vertexCoords[2]);
                this.vBuffer[3*this.numVertices+2] = parseFloat(vertexCoords[3]);
                this.numVertices += 1;
            }
            
            else if(lines[i][0]=='f')
            {
                var faceVertices = lines[i].split(" ");
                this.fBuffer[3*this.numFaces] = parseInt(faceVertices[2]-1);
                this.fBuffer[3*this.numFaces+1] = parseInt(faceVertices[3]-1);
                this.fBuffer[3*this.numFaces+2] = parseInt(faceVertices[4]-1);
                this.numFaces += 1;
            }
            else
            {
